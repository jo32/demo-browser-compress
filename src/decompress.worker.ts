...
const logger = getLogger("DecompressWorker");

class CustromInflate extends pako.Inflate {
...
class DecompressWorker extends FileTranformingWorker {

  __inflator: CustromInflate | undefined;
  ...
  onMessage(data: FileTranformingWorkerReceivedPaylaod) {
    ...
    switch (evt) {
      case "init":
        ...
        this.__inflator = new CustromInflate(this);
        postMessage(["inited"]);
      ...
      case "push":
        ...
        while (this.__readOffset < this.__fileLength) {
          ...
          this.__inflator!.push(readView, isFinished);
...
const dw = new DecompressWorker();

// Respond to message from parent thread
ctx.addEventListener("message", (event) => {
  dw.onMessage(event.data);
});
